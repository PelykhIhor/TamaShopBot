...
bot = telebot.TeleBot('8023969883:AAF2eqluMfaQrUdBWMYiOo3zM8Rs6gY1KVo')
...
def complete_purchase(message, product_index, discount, promo_code):
    ...
    orders[message.chat.id].append(product)

    if message.chat.id not in loyalty_points:
        loyalty_points[message.chat.id] = 0
    loyalty_points[message.chat.id] += int(discounted_price * 0.01)
...
@bot.message_handler(
    func=lambda message: message.text == '🎁 Програма лояльності')
def show_loyalty_program(message):
    ...
    response += "⭐️ 500 балів: 5% знижка на наступну покупку 🎟️\n"
    response += "🌈 1000 балів: 10% знижка на наступну покупку 🌟\n"
    response += "🎁 2000 балів: Ексклюзивний аксесуар у подарунок 📱\n"
    response += "🔖 3000 балів: Індивідуальне гравірування на аксесуарах ✏️\n"
    response += "📅 5000 балів: Доступ до новинок на день раніше ⏰\n"
    response += "🌐 10000 балів: Віртуальна екскурсія Apple Store в Каліфорнії з гідом 🗺️\n"
    response += "🏠 12000 балів: Візит техніка для налаштування техніки вдома 👨‍💻\n"
    response += "🎟️ 15000 балів: Участь у розіграші на запрошення на офіційну презентацію Apple 🎬\n"
    response += "🎶 20000 балів: Річна підписка на Apple Music або Apple TV+ 🎧\n"
    response += "👑 30000 балів: Членство в клубі 'Apple Insiders' з доступом до привілеїв та знижок 💼\n"
